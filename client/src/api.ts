...
export const RosterApiAxiosParamCreator = function (
  ...
  return {
     ...
     * @throws {RequiredError}
     */
    updateRosterTemplate: async (
      id: number,
      params?: RosterTemplateUpdateParams,
      options: RawAxiosRequestConfig = {},
    ): Promise<RequestArgs> => {
      // verify required parameter 'id' is not null or undefined
...
export const RosterApiFp = function (configuration?: Configuration) {
  ...
  return {
    ...
    > {
      const localVarAxiosArgs =
        await localVarAxiosParamCreator.updateRosterTemplate(
          id,
          params,
          options,
        );
      const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
      const localVarOperationServerBasePath =
        operationServerMap["RosterApi.updateRosterTemplate"]?.[
          localVarOperationServerIndex
        ]?.url;
      return (axios, basePath) =>
        createRequestFunction(

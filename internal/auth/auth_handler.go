...
func NewAuthHandler(rg *gin.RouterGroup, auth Service, provider *oidc.Provider, config *oauth2.Config) *Handler {
	h := &Handler{config: config, provider: provider, service: auth}
